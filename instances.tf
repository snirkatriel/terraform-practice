...
resource "aws_instance" "jenkins-worker-oregon" {
  ...
  associate_public_ip_address = true
  vpc_security_group_ids      = [aws_security_group.jenkins-sg-node.id]
  subnet_id                   = aws_subnet.subnet_1_node.id
  ...
  provisioner "local-exec" {
    command = <<EOF
aws --profile ${var.profile} ec2 wait instance-status-ok --region ${var.region-node} --instance-ids ${self.id}
